...
The integration builds five independent libFuzzer entrypoints:
...
- `cookie_parse` exercises Cookie-field slicing, asserts borrowed slices stay inside the parsed input, and cross-checks the vectorized jar against the scalar iterator.
...
in CI. ClusterFuzzLite's bad-build check verifies that all five targets link
