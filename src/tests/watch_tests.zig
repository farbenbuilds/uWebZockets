...
test "watch paths validate shape and platform" {
    if (file_watch.available) {
        ...
        const unavailable = config.ServerConfig{
            .enable_dev_log = true,
            .watch_paths = &.{"src"},
        };
        try testing.expectError(error.WatchUnavailable, unavailable.validate());
    }
}
