...
[ -d "$lsquic_root" ] || fail "missing $lsquic_root; check out the vendor submodules"

tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/uwebzockets-overlay.XXXXXX")
trap 'rm -rf "$tmp_dir"' 0 1 2 15
mkdir -p "$tmp_dir/src/liblsquic"

for file in $overlay_files; do
    cp "$lsquic_root/src/liblsquic/$file" "$tmp_dir/src/liblsquic/$file"
done
(
    cd "$tmp_dir"
...
for file in $overlay_files; do
    cmp -s "$tmp_dir/src/liblsquic/$file" "$overlay_root/src/liblsquic/$file" || fail \
...
versions=$(sed -n '/^enum lsquic_version/,/^};/p' "$lsquic_root/include/lsquic.h" |
