...
\begin{tikzpicture}[
  font=\scriptsize,
  cell/.style={draw, minimum height=0.48cm, minimum width=0.98cm, align=center, font=\tiny},
  cache/.style={draw=black!65, dashed, thick, inner sep=3pt}
]
\node[anchor=west] at (0,2.0) {Conventional layout (without DoD)};
\foreach \i/\txt in {0/{id},1/{state},2/{window},4/{id},5/{state},6/{window}} {
  \node[cell,fill=blue!18] (a\i) at (\i*1.0,1.45) {\txt};
}
\foreach \i/\txt in {3/{cold},7/{cold}} {
  \node[cell,fill=gray!25] (a\i) at (\i*1.0,1.45) {\txt};
}
\node[cache, fit=(a0)(a3)] (ca) {};
\node[cache, fit=(a4)(a7)] (cb) {};
\node[below=0.16cm of ca] {cache line A};
\node[below=0.16cm of cb] {cache line B};
\node[anchor=west] at (0,0.45) {Data-oriented layout (DoD / SoA)};
\foreach \i/\txt in {0/{id$_0$},1/{id$_1$},2/{id$_2$},3/{id$_3$},4/{id$_4$},5/{id$_5$},6/{id$_6$},7/{id$_7$}} {
  \node[cell,fill=blue!18] (s\i) at (\i*1.0,-0.10) {\txt};
}
\node[cache, fit=(s0)(s7)] (cs) {};
\node[below=0.16cm of cs] {one dense identifier scan};
\foreach \i/\txt in {0/{state$_0$},1/{state$_1$},2/{state$_2$},3/{state$_3$},4/{state$_4$},5/{state$_5$},6/{state$_6$},7/{state$_7$}} {
  \node[cell,fill=gray!25] at (\i*1.0,-1.25) {\txt};
}
\foreach \i/\txt in {0/{window$_0$},1/{window$_1$},2/{window$_2$},3/{window$_3$},4/{window$_4$},5/{window$_5$},6/{window$_6$},7/{window$_7$}} {
  \node[cell,fill=gray!18] at (\i*1.0,-2.35) {\txt};
}
\node[anchor=west,align=left,text width=3.2cm] at (8.15,-1.25) {columns selected by access pattern};
\node[anchor=west] at (8.15,1.45) {\textcolor{blue!70!black}{\rule{0.35cm}{0.4pt}} hot field};
\node[anchor=west] at (8.15,1.10) {\textcolor{gray!70!black}{\rule{0.35cm}{0.4pt}} cold field};
\end{tikzpicture}
\caption{Conceptual comparison of a conventional cache layout without data-oriented design (DoD) and a DoD/SoA layout. The conventional cache lines interleave hot and cold fields, whereas DoD isolates field families so a hot identifier scan can remain dense. Blue marks frequently accessed fields for the illustrated scan; gray marks colder fields. This is an access-pattern hypothesis, not a processor trace.}
\label{fig:memory}
\end{figure*}
...
\begin{scope}[x=0.006cm,y=5cm]
  ...
  \node[anchor=west,align=left,text width=3.2cm,fill=white,inner sep=1.5pt]
    at (520,0.47) {\scriptsize Synthetic placeholder;\\ eBPF socket-hook timestamps};
  \node[anchor=west,fill=white,inner sep=1.5pt]
    at (520,0.32) {\scriptsize\textcolor{blue}{\rule{0.35cm}{0.4pt} Packed slab}};
  \node[anchor=west,fill=white,inner sep=1.5pt]
    at (520,0.23) {\scriptsize\textcolor{red}{\rule{0.35cm}{0.4pt} AoS reference}};
\end{scope}
\end{tikzpicture}
