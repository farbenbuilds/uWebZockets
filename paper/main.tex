...
\begin{lstlisting}[language=C,caption={Source derived form of the bounded O(1) freelist operations},label={lst:freelist}]
...
\begin{lstlisting}[language=C,caption={Structure of arrays used by the bounded HTTP/2 stream slab},label={lst:soa}]
...
\begin{lstlisting}[language=C,caption={Source derived form of vectorized WebSocket masking},label={lst:simd}]
...
I would measure tail latency with a closed loop client and an open loop overload experiment. The closed loop experiment would estimate service time without queue instability. The open loop experiment would expose admission, output backpressure, and event loop scheduling effects. Each request would carry a trace identifier from socket readiness through parsing, dispatch, response enqueue, final write, and close. An eBPF hook or equivalent tracing point would record timestamps at these boundaries without modifying allocator policy \cite{ebpf}. I would report median, 95th percentile, 99th percentile, and 99.9th percentile latency, along with queue depth and capacity rejection rate.
...
\begin{scope}[x=0.9cm,y=6cm]
  \foreach \y in {0.1,0.2,0.3,0.4} {\draw[gray!20] (0,\y) -- (7.0,\y);}
  \draw[->] (0,0) -- (7.1,0) node[below right,xshift=-1.1cm] {\scriptsize Latency bucket, microseconds};
  \draw[->] (0,0) -- (0,0.49) node[above,rotate=90,yshift=0.25cm] {\scriptsize Requests, normalized};
  \foreach \x/\label in {0.55/80,1.55/160,2.55/240,3.55/320,4.55/400,6.05/800} {
    \node[below] at (\x+0.12,0) {\scriptsize \label};
  }
  \foreach \x/\a/\b in {0.40/0.38/0.32,1.40/0.31/0.26,2.40/0.20/0.18,3.40/0.10/0.11,4.40/0.05/0.07,5.90/0.02/0.06} {
    \draw[fill=blue!45,draw=blue!75] (\x,0) rectangle ++(0.20,\a);
    \draw[fill=red!45,draw=red!75] (\x+0.22,0) rectangle ++(0.20,\b);
  }
  \node[anchor=west] at (4.3,0.46) {\scriptsize\textcolor{blue}{\rule{0.35cm}{0.4pt} Packed slab placeholder}};
  \node[anchor=west] at (4.3,0.40) {\scriptsize\textcolor{red}{\rule{0.35cm}{0.4pt} AoS placeholder}};
\end{scope}
\end{tikzpicture}
\caption{Synthetic design placeholder for tail analysis using eBPF-style event observations. The values are not observations and do not establish a measured latency advantage.}
\label{fig:tail}
\end{figure}
